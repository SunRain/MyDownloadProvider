...
public abstract class MemoryCache<K, V> {
	 ...
	 */
	int setMemCacheSizePercent(float percent) {
	...
	public MemoryCache(float cacheSizePercent) {
		...
		int cacheSize = setMemCacheSizePercent(defaultPercent);
		mLruCache = new LruCache<K, V>(cacheSize) {
			...
			protected void entryRemoved(boolean evicted, K key, V oldValue,
					V newValue) {
				// MyLog.getLogger().d(
				// "mSoftCache.size() = " + mSoftCache.size()
	...
	public void clearCache() {
		MyLog.i("mLruCache = " + mLruCache.size() + ",mSoftCache = "
				+ mSoftCache.size());
		synchronized (mLruCache) {
			mLruCache.evictAll();
		}
